...
-Goal-
Given a text and a list of entity types, extract entities of those types and their relationships. Use {language} as output.
-Steps-
1. Extract Entities
   For each entity, output:
   ("entity"{tuple_delimiter}<entity_name>{tuple_delimiter}<entity_type>{tuple_delimiter}<entity_description>)
   - entity_name: As in text (capitalize if English)
   - entity_type: One of [{entity_types}]
   - entity_description: Summary of role/traits/activities
2. Extract Relationships
   For each related entity pair, output:
   ...
   - relationship_keywords: Themes or concepts
   - relationship_strength: Numeric (e.g., 1–10)
3. Content Keywords
   Summarize main themes in:
   ("content_keywords"{tuple_delimiter}<keywords>)
4. Return all in {language}, delimited by {record_delimiter}.
...
Text:
Alex clenched his jaw, tense under Taylor’s authority. He and Jordan shared a quiet defiance of Cruz’s control.
Taylor paused, eyeing the device. “If this tech can be understood... it could change everything.”
The tension eased. Jordan met Taylor’s gaze. A silent truce formed. Alex noted it.

Output:
("entity"{tuple_delimiter}"Alex"{tuple_delimiter}"person"{tuple_delimiter}"Frustrated, aligned with Jordan against Cruz."){record_delimiter}
("entity"{tuple_delimiter}"Taylor"{tuple_delimiter}"person"{tuple_delimiter}"Authoritative, then reverent toward the device."){record_delimiter}
("entity"{tuple_delimiter}"Jordan"{tuple_delimiter}"person"{tuple_delimiter}"Discovery-driven, bridges with Taylor."){record_delimiter}
("entity"{tuple_delimiter}"Cruz"{tuple_delimiter}"person"{tuple_delimiter}"Represents control, opposed by others."){record_delimiter}
("entity"{tuple_delimiter}"The Device"{tuple_delimiter}"technology"{tuple_delimiter}"Valued tech, possibly transformative."){record_delimiter}
("relationship"{tuple_delimiter}"Alex"{tuple_delimiter}"Taylor"{tuple_delimiter}"Reacting to authority, notes change."{tuple_delimiter}"tension, observation"{tuple_delimiter}7){record_delimiter}
("relationship"{tuple_delimiter}"Alex"{tuple_delimiter}"Jordan"{tuple_delimiter}"Bond over defiance of Cruz."{tuple_delimiter}"shared purpose"{tuple_delimiter}6){record_delimiter}
("relationship"{tuple_delimiter}"Taylor"{tuple_delimiter}"Jordan"{tuple_delimiter}"Truce via shared respect for tech."{tuple_delimiter}"reconciliation"{tuple_delimiter}8){record_delimiter}
("relationship"{tuple_delimiter}"Jordan"{tuple_delimiter}"Cruz"{tuple_delimiter}"Jordan opposes Cruz’s vision."{tuple_delimiter}"conflict"{tuple_delimiter}5){record_delimiter}
("relationship"{tuple_delimiter}"Taylor"{tuple_delimiter}"The Device"{tuple_delimiter}"Respects its potential."{tuple_delimiter}"tech value"{tuple_delimiter}9){record_delimiter}
("content_keywords"{tuple_delimiter}"conflict, tech, truce, rebellion"){completion_delimiter}
#############################""",
]
